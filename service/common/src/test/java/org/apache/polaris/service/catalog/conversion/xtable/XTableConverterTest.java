...
class XTableConverterTest {
    ...
    private static final String REQUEST_BODY_DELTA_SOURCE = "{\"source-format\":\"DELTA\", \"target-format\":\"ICEBERG\", \"source-metadata-path\":\"" + DELTA_METADATA_PATH + "\"}";
    private static final String RESPONSE_BODY_ICEBERG_TARGET = "{\"target-metadata-path\":\"" + ICEBERG_METADATA_PATH + "\"}";
    private static final String REQUEST_BODY_ICEBERG_SOURCE = "{\"source-format\":\"ICEBERG\", \"target-format\":\"DELTA\", \"source-metadata-path\":\"" + ICEBERG_METADATA_PATH + "\"}";
    private static final String RESPONSE_BODY_DELTA_TARGET = "{\"target-metadata-path\":\"" + DELTA_METADATA_PATH + "\"}";
    ...
    void executeConversionOnGenericTable() throws Exception {
        ...
        when(mockMapper.readValue(RESPONSE_BODY_ICEBERG_TARGET, RunSyncResponse.class)).thenReturn(expected);
    ...
    void executeConversionOnIcebergTable() throws Exception {
        ...
        when(mockMapper.readValue(RESPONSE_BODY_DELTA_TARGET, RunSyncResponse.class)).thenReturn(expected);
    ...
    void executeConversionFailure() throws Exception {
        ...
        IllegalStateException exception = assertThrows(
                ...
                () -> converter.execute(genericTableEntity)
        );
